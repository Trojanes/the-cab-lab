...
/** World-space corners and axis-aligned bounds of the cabinet envelope for a pose. */
export function envelopeFootprint(cab, pose) {
  const env = envelopeBox(cab, resultFor(cab.id));
  const locals = [
    [env.x0, env.y0, env.z0], [env.x1, env.y0, env.z0], [env.x1, env.y1, env.z0], [env.x0, env.y1, env.z0],
    [env.x0, env.y0, env.z1], [env.x1, env.y0, env.z1], [env.x1, env.y1, env.z1], [env.x0, env.y1, env.z1],
  ];
